...
def get_parser():
    parser = argparse.ArgumentParser(description="UNINEXT demo for builtin configs")
    parser.add_argument(
        "--config-file",
        default="projects/UNINEXT/configs/image_joint_r50_dino_load_obj365.yaml",
        metavar="FILE",
...
if __name__ == "__main__":
    ...
    if args.output:
        if "." in os.path.split(args.output)[-1]:
            # args.output is a file
            os.makedirs(os.path.split(args.output)[0], exist_ok=True)
        else:
            # args.output is a dir
